...
show_pending_assignments_for_node() {
  ...
  local db_path="$2"
  local node_name="$3"
  "${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node "${node_name}"
}
...
runtime_model_path="/naim/shared/models/live-single-gpu/${model_filename}"
...
"${build_dir}/naim-controller" show-rollout-actions --db "${db_path}" --node node-a | grep -F \
...
first_rollout_action_id="$("${build_dir}/naim-controller" show-rollout-actions --db "${db_path}" --node node-a | sed -n 's/^  - id=\([0-9][0-9]*\).*/\1/p' | head -n 1)"
second_rollout_action_id="$("${build_dir}/naim-controller" show-rollout-actions --db "${db_path}" --node node-a | sed -n 's/^  - id=\([0-9][0-9]*\).*/\1/p' | sed -n '2p')"
test -n "${first_rollout_action_id}"
test -n "${second_rollout_action_id}"

echo "[live-single-gpu] enqueueing live eviction"
"${build_dir}/naim-controller" enqueue-rollout-eviction --db "${db_path}" --id "${first_rollout_action_id}" | grep -F \
...
"${build_dir}/naim-controller" reconcile-rollout-actions --db "${db_path}" --artifacts-root "${artifacts_root}" | grep -F \
  "applied ready rollout action id=${second_rollout_action_id}" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "desired generation: 2" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F \
  "worker-a role=worker node=node-a gpu=0 fraction=1 placement_mode=movable share_mode=exclusive" >/dev/null
if "${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "worker-b role=worker" >/dev/null; then
...
if "${build_dir}/naim-controller" show-host-observations --db "${db_path}" | grep -F \
...
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" | grep -F "gpu device=0" >/dev/null
