...
"${build_dir}/naim-controller" show-hostd-hosts --db "${launcher_db_path}" --node gpu-b | grep -F '"node_name": "gpu-b"' >/dev/null
test "$(
...
test ! -f "${launcher_systemd_dir}/naim-node-controller.service"
test ! -f "${launcher_systemd_dir}/naim-node-hostd.service"
"${build_dir}/naim-controller" init-db --db "${remote_agent_db_path}" >/dev/null
...
"${build_dir}/naim-controller" serve --db "${remote_agent_db_path}" --listen-host 127.0.0.1 --listen-port "${remote_http_port}" >/tmp/naim-controller-remote-agent.log 2>&1 &
http_server_pid="$!"
wait_for_http "http://127.0.0.1:${remote_http_port}/health" 100 "/tmp/naim-controller-remote-agent.log"
...
"${build_dir}/naim-controller" show-hostd-hosts --db "${remote_agent_db_path}" --node node-a | grep -F '"session_state": "connected"' >/dev/null
remote_agent_observations="$("${build_dir}/naim-controller" show-host-observations --db "${remote_agent_db_path}" --node node-a)"
...
remote_agent_events="$("${build_dir}/naim-controller" show-events --db "${remote_agent_db_path}" --node node-a --category host-assignment 2>/dev/null || true)"
if printf '%s' "${remote_agent_events}" | grep -F 'category=host-assignment type=applied' >/dev/null; then
  :
else
  "${build_dir}/naim-controller" show-host-assignments --db "${remote_agent_db_path}" --node node-a | grep -F '(empty)' >/dev/null
fi
"${build_dir}/naim-node" install hostd \
  --controller "http://127.0.0.1:${remote_http_port}" \
  --node node-a \
...
"${build_dir}/naim-controller" show-hostd-hosts --db "${remote_agent_db_path}" --node node-a | grep -F '"session_state": "rotation-pending"' >/dev/null
...
"${build_dir}/naim-hostd" report-observed-state \
  ...
  --state-root "${remote_agent_state_root}" >/dev/null
"${build_dir}/naim-controller" show-hostd-hosts --db "${remote_agent_db_path}" --node node-a | grep -F '"session_state": "connected"' >/dev/null
"${build_dir}/naim-controller" revoke-hostd --db "${remote_agent_db_path}" --node node-a >/dev/null
"${build_dir}/naim-controller" show-hostd-hosts --db "${remote_agent_db_path}" --node node-a | grep -F '"registration_state": "revoked"' >/dev/null
if "${build_dir}/naim-hostd" report-observed-state \
  --controller "http://127.0.0.1:${remote_http_port}" \
  --node node-a \
  --host-private-key "${remote_agent_rotated_layout_state_root}/keys/hostd.key.b64" \
...
"${build_dir}/naim-controller" show-demo-plan >/dev/null
"${build_dir}/naim-controller" render-demo-compose --node node-a >/dev/null
...
"${build_dir}/naim-controller" serve --db "${db_path}" --listen-host 127.0.0.1 --listen-port "${http_port}" --ui-root "${ui_smoke_root}" >/tmp/naim-controller-serve.log 2>&1 &
http_server_pid="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${http_port}/health" >/tmp/naim-controller-health.json 2>/dev/null; then
    break
  fi
  sleep 0.1
done
wait_for_http "http://127.0.0.1:${http_port}/health" 100 "/tmp/naim-controller-serve.log"
...
curl -fsS "http://127.0.0.1:${http_port}/planes/alpha" | grep -F 'Naim Command Deck' >/dev/null
curl -fsS "http://127.0.0.1:${http_port}/health" | grep -F '"service":"naim-controller"' >/dev/null
...
grep -F 'NAIM_CONTROLLER_UPSTREAM: http://host.docker.internal:18080' "${web_ui_runtime_root}/docker-compose.yml" >/dev/null
"${build_dir}/naim-controller" show-web-ui-status --web-ui-root "${web_ui_runtime_root}" | grep -F 'materialized=yes' >/dev/null
"${build_dir}/naim-controller" show-web-ui-status --web-ui-root "${web_ui_runtime_root}" | grep -F 'running=no' >/dev/null
"${build_dir}/naim-controller" stop-web-ui --db "${db_path}" --web-ui-root "${web_ui_runtime_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-web-ui-status --web-ui-root "${web_ui_runtime_root}" | grep -F 'materialized=no' >/dev/null
"${build_dir}/naim-controller" show-events --db "${db_path}" --category web-ui | grep -F 'category=web-ui type=stopped' >/dev/null
"${build_dir}/naim-controller" init-db --db "${api_db_path}" >/dev/null
http_api_port="$("${script_dir}/naim-devtool.sh" free-port)"
"${build_dir}/naim-controller" serve --db "${api_db_path}" --listen-host 127.0.0.1 --listen-port "${http_api_port}" >/tmp/naim-controller-api-mutations.log 2>&1 &
http_server_pid="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${http_api_port}/health" >/tmp/naim-controller-api-mutations-health.json 2>/dev/null; then
    break
  fi
  sleep 0.1
done
wait_for_http "http://127.0.0.1:${http_api_port}/health" 100 "/tmp/naim-controller-api-mutations.log"
sse_bundle_output="/tmp/naim-controller-events-stream-bundle.txt"
sse_replay_output="/tmp/naim-controller-events-stream-replay.txt"
rm -f "${sse_bundle_output}" "${sse_replay_output}"
curl -NsS --max-time 5 "http://127.0.0.1:${http_api_port}/api/v1/events/stream?category=bundle" >"${sse_bundle_output}" 2>/tmp/naim-controller-events-stream-bundle.log &
sse_bundle_pid="$!"
sleep 0.2
...
api_stop_assignment_output="$("${build_dir}/naim-hostd" apply-next-assignment --node node-a --db "${api_db_path}" --runtime-root "${api_runtime_root}" --state-root "${api_state_root}" --compose-mode skip)"
...
api_local_state_output="$("${build_dir}/naim-hostd" show-local-state --node node-a --state-root "${api_state_root}")"
...
curl -NsS --max-time 5 -H "Last-Event-ID: ${last_seen_event_id}" "http://127.0.0.1:${http_api_port}/api/v1/events/stream?node=node-a" >"${sse_replay_output}" 2>/tmp/naim-controller-events-stream-replay.log &
sse_replay_pid="$!"
sleep 0.2
...
"${build_dir}/naim-controller" show-state --controller "http://127.0.0.1:${http_api_port}" | grep -F '"desired_generation": 2' >/dev/null
"${build_dir}/naim-controller" show-host-assignments --controller "http://127.0.0.1:${http_api_port}" --node node-a | grep -F '"assignments"' >/dev/null
"${build_dir}/naim-controller" show-node-availability --controller "http://127.0.0.1:${http_api_port}" --node node-b | grep -F '"availability": "unavailable"' >/dev/null
"${build_dir}/naim-controller" validate-bundle --controller "http://127.0.0.1:${http_api_port}" --bundle "${PWD}/config/demo-plane" | grep -F 'bundle validation: OK' >/dev/null
"${build_dir}/naim-controller" set-node-availability --controller "http://127.0.0.1:${http_api_port}" --node node-a --availability draining --message cli-http | grep -F 'updated node availability for node-a' >/dev/null
for _ in $(seq 1 30); do
...
NAIM_CONTROLLER="http://127.0.0.1:${http_api_port}" "${build_dir}/naim-controller" show-node-availability --node node-a | grep -F '"availability": "draining"' >/dev/null
kill "${http_server_pid}" >/dev/null 2>&1 || true
wait "${http_server_pid}" >/dev/null 2>&1 || true
http_server_pid=""
"${build_dir}/naim-controller" validate-bundle --bundle "${PWD}/config/demo-plane" >/dev/null
...
if "${build_dir}/naim-controller" validate-bundle --bundle "${invalid_bundle_dir}" >/tmp/naim-invalid-bundle.txt 2>&1; then
  echo "check: expected invalid scheduler bundle to be rejected" >&2
  exit 1
fi
grep -F "mixes share_mode=exclusive with other workers" /tmp/naim-invalid-bundle.txt >/dev/null
grep -F "gpu memory oversubscription" /tmp/naim-invalid-bundle.txt >/dev/null
...
"${build_dir}/naim-controller" validate-bundle --bundle "${auto_bundle_dir}" | grep -F "node=node-a gpu=1 workers=worker-b exclusive=worker-b" >/dev/null
...
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_bundle_dir}" | grep -F "preemption-hints:" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_bundle_dir}" | grep -F "node=node-a gpu=0 victims=worker-b reason=protect guaranteed workers before best-effort workers on the same GPU" >/dev/null
...
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "action=preempt-best-effort-to-exclusive" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "victims=worker-b" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "worker=worker-a decision=deferred next_action=preempt-best-effort-to-exclusive next_target=node-a:0 victims=worker-b defer_reason=requires-controlled-preemption" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "rollout-actions:" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "step=1 worker=worker-a action=evict-best-effort target=node-a:0 victims=worker-b" >/dev/null
"${build_dir}/naim-controller" validate-bundle --bundle "${preemption_candidate_bundle_dir}" | grep -F "step=2 worker=worker-a action=retry-placement target=node-a:0" >/dev/null
preemption_db_path="${PWD}/var/controller-preemption.sqlite"
cmake -E rm -f "${preemption_db_path}"
"${build_dir}/naim-controller" init-db --db "${preemption_db_path}" >/dev/null
"${build_dir}/naim-controller" import-bundle --bundle "${preemption_candidate_bundle_dir}" --db "${preemption_db_path}" | grep -F "rollout-gates:" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "message=scheduler rollout actions pending on target node node-a for workers worker-a" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${preemption_db_path}" | grep -F "worker=worker-a placement_mode=movable current=node-b:0 class=rollout-class decision=hold state=active-rollout target=node-a:0" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${preemption_db_path}" | grep -F "rebalance-loop-status:" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${preemption_db_path}" | grep -F "state=waiting-for-convergence reason=unconverged-nodes=2" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${preemption_db_path}" | grep -F "actionable=0 safe_direct=0 rollout_class=1 gated=0 blocked_active_rollouts=1 assignment_busy=0 observation_gated=0 stable_holds=0 below_threshold=0 deferred=0 no_candidate=0" >/dev/null
"${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | grep -F "id=" >/dev/null
"${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | grep -F "step=1 worker=worker-a action=evict-best-effort target=node-a:0 status=pending victims=worker-b" >/dev/null
"${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | grep -F "step=2 worker=worker-a action=retry-placement target=node-a:0 status=pending" >/dev/null
"${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | grep -F "phase=planned" >/dev/null
first_rollout_action_id="$("${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | sed -n 's/^  - id=\([0-9][0-9]*\).*/\1/p' | head -n 1)"
second_rollout_action_id="$("${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" --node node-a | sed -n 's/^  - id=\([0-9][0-9]*\).*/\1/p' | sed -n '2p')"
test -n "${first_rollout_action_id}"
test -n "${second_rollout_action_id}"
http_preemption_port="$("${script_dir}/naim-devtool.sh" free-port)"
"${build_dir}/naim-controller" serve --db "${preemption_db_path}" --listen-host 127.0.0.1 --listen-port "${http_preemption_port}" >/tmp/naim-controller-preemption-serve.log 2>&1 &
http_server_pid="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${http_preemption_port}/api/v1/rollout-actions?node=node-a" >/tmp/naim-controller-preemption-api.json 2>/dev/null; then
    break
  fi
  sleep 0.1
done
wait_for_http "http://127.0.0.1:${http_preemption_port}/api/v1/rollout-actions?node=node-a" 100 "/tmp/naim-controller-preemption-serve.log"
...
"${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "type=evict-workers status=pending" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${preemption_db_path}" --node node-a --runtime-root "${preemption_runtime_root}" --state-root "${preemption_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "type=evict-workers status=applied" >/dev/null
...
"${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "desired generation: 2" >/dev/null
"${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "worker-a role=worker node=node-a gpu=0 fraction=1 placement_mode=movable share_mode=exclusive priority=200 preemptible=false memory_cap_mb=12288 placement=auto placement_action=materialized-retry-placement placement_score=22 placement_decision=applied" >/dev/null
"${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "decision=applied" >/dev/null
"${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "phase=retry-materialized" >/dev/null
if "${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "worker-b role=worker" >/dev/null; then
  echo "check: expected worker-b to be evicted from desired state after materialized retry placement" >&2
  exit 1
fi
"${build_dir}/naim-controller" show-rollout-actions --db "${preemption_db_path}" | grep -F "(empty)" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "generation=2 attempts=0/3 type=apply-node-state status=pending" >/dev/null
if "${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "scheduler rollout actions pending on target node" >/dev/null; then
...
http_server_pid=""
"${build_dir}/naim-hostd" apply-next-assignment --db "${preemption_db_path}" --node node-a --runtime-root "${preemption_runtime_root}" --state-root "${preemption_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${preemption_db_path}" --node node-a | grep -F "generation=2 attempts=1/3 type=apply-node-state status=applied" >/dev/null
"${build_dir}/naim-controller" show-state --db "${preemption_db_path}" | grep -F "phase=rollout-applied" >/dev/null
...
"${build_dir}/naim-controller" validate-bundle --bundle "${movable_bundle_dir}" | grep -F "worker=worker-b decision=proposed next_action=upgrade-to-exclusive next_target=node-a:1" >/dev/null
"${build_dir}/naim-controller" init-db --db "${rebalance_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${movable_bundle_dir}" --db "${rebalance_db_path}" --artifacts-root "${rebalance_artifacts_root}" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${rebalance_db_path}" --artifacts-root "${rebalance_artifacts_root}" | grep -F "cluster_ready=no active_rollouts=0 blocking_assignment_nodes=2 unconverged_nodes=2" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${rebalance_db_path}" --artifacts-root "${rebalance_artifacts_root}" | grep -F "state=waiting-for-convergence reason=unconverged-nodes=2" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${rebalance_db_path}" --artifacts-root "${rebalance_artifacts_root}" | grep -F "rebalance proposals: blocked by controller gate" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=gated decision=hold state=assignment-in-flight target=node-a:1 action=upgrade-to-exclusive" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${rebalance_db_path}" --node node-a --runtime-root "${rebalance_runtime_root}" --state-root "${rebalance_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${rebalance_db_path}" --node node-b --runtime-root "${rebalance_runtime_root}" --state-root "${rebalance_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=safe-direct decision=propose state=ready-move target=node-a:1 action=upgrade-to-exclusive" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "cluster_ready=yes active_rollouts=0 blocking_assignment_nodes=0 unconverged_nodes=0" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "state=actionable reason=safe-direct-workers=1" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "actionable=1 safe_direct=1 rollout_class=0 gated=0 blocked_active_rollouts=0 assignment_busy=0 observation_gated=0 stable_holds=0 below_threshold=0 deferred=0 no_candidate=0" >/dev/null
http_rebalance_port="$("${script_dir}/naim-devtool.sh" free-port)"
"${build_dir}/naim-controller" serve --db "${rebalance_db_path}" --listen-host 127.0.0.1 --listen-port "${http_rebalance_port}" >/tmp/naim-controller-rebalance-serve.log 2>&1 &
http_server_pid="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${http_rebalance_port}/api/v1/rebalance-plan" >/tmp/naim-controller-rebalance-api.json 2>/dev/null; then
    break
  fi
  sleep 0.1
done
wait_for_http "http://127.0.0.1:${http_rebalance_port}/api/v1/rebalance-plan" 100 "/tmp/naim-controller-rebalance-serve.log"
...
wait "${http_server_pid}" >/dev/null 2>&1 || true
http_server_pid=""
"${build_dir}/naim-controller" show-state --db "${rebalance_db_path}" | grep -F "desired generation: 2" >/dev/null
"${build_dir}/naim-controller" show-state --db "${rebalance_db_path}" | grep -F "worker-b role=worker node=node-a gpu=1 fraction=1 placement_mode=movable share_mode=exclusive priority=100 preemptible=true memory_cap_mb=8192 placement=auto placement_action=materialized-rebalance-upgrade-to-exclusive placement_score=145 placement_decision=applied" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${rebalance_db_path}" | grep -F "plane_action=rebalance phase=verifying-move worker=worker-b generation=2 stable_samples=0/3 rollback_attempts=0" >/dev/null
...
{
  "name": "alpha",
  "control_root": "/naim/shared/control/alpha",
  "runtime": {
    "primary_infer_node": "node-a",
    "net_if": "eth0",
    "models_root": "/naim/shared/models",
    "gguf_cache_dir": "/naim/shared/models/gguf",
    "infer_log_dir": "/naim/shared/logs/infer",
    "llama_port": 8000,
    "llama_ctx_size": 8192,
    "llama_threads": 8,
    "llama_gpu_layers": 99,
...
EOF
"${build_dir}/naim-controller" init-db --db "${budget_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${budget_bundle_dir}" --db "${budget_db_path}" --artifacts-root "${budget_artifacts_root}" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-a --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-b --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-c --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "iteration=0/1 exhausted=no" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "state=actionable reason=safe-direct-workers=2" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=safe-direct decision=propose state=ready-in-place-upgrade target=node-b:1 action=upgrade-to-exclusive score=185 preemption_required=no" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "worker=worker-c placement_mode=movable current=node-c:0 class=safe-direct decision=propose state=ready-in-place-upgrade target=node-c:1 action=upgrade-to-exclusive score=185 preemption_required=no" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${budget_db_path}" --artifacts-root "${budget_artifacts_root}" | grep -F "applied rebalance proposal for worker 'worker-b'" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-a --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-b --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${budget_db_path}" --node node-c --runtime-root "${budget_runtime_root}" --state-root "${budget_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "iteration=1/1 exhausted=yes" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "state=waiting-for-convergence reason=active-rollouts=1" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "worker=worker-c placement_mode=movable current=node-c:0 class=gated decision=hold state=active-scheduler-action" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${budget_db_path}" | grep -F "plane_action=rebalance phase=verifying-move worker=worker-b generation=2 stable_samples=0/3 rollback_attempts=0" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${budget_db_path}" --artifacts-root "${budget_artifacts_root}" | grep -F "rebalance proposals: blocked by controller gate" >/dev/null
...
"${build_dir}/naim-controller" init-db --db "${threshold_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${threshold_bundle_dir}" --db "${threshold_db_path}" --artifacts-root "${threshold_artifacts_root}" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${threshold_db_path}" --node node-a --runtime-root "${threshold_runtime_root}" --state-root "${threshold_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${threshold_db_path}" --node node-b --runtime-root "${threshold_runtime_root}" --state-root "${threshold_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${threshold_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=stable decision=hold state=below-threshold target=node-a:0 action=move-with-current-fraction score=32 preemption_required=no gate_reason=score-below-threshold(32<100)" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${threshold_db_path}" | grep -F "state=complete reason=remaining-moves-below-threshold=1" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${threshold_db_path}" | grep -F "actionable=0 safe_direct=0 rollout_class=0 gated=0 blocked_active_rollouts=0 assignment_busy=0 observation_gated=0 stable_holds=1 below_threshold=1 deferred=0 no_candidate=0" >/dev/null
"${build_dir}/naim-controller" reconcile-rebalance-proposals --db "${threshold_db_path}" --artifacts-root "${threshold_artifacts_root}" | grep -F "rebalance proposals: none actionable" >/dev/null
...
"${build_dir}/naim-controller" init-db --db "${drain_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${drain_bundle_dir}" --db "${drain_db_path}" --artifacts-root "${drain_artifacts_root}" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${drain_db_path}" --node node-a --runtime-root "${drain_runtime_root}" --state-root "${drain_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${drain_db_path}" --node node-b --runtime-root "${drain_runtime_root}" --state-root "${drain_state_root}" --compose-mode skip >/dev/null
...
SQL
"${build_dir}/naim-controller" show-rebalance-plan --db "${drain_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=safe-direct decision=propose state=ready-drain-move target=node-a:1 action=upgrade-to-exclusive" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${drain_db_path}" | grep -F "actionable=1 safe_direct=1 rollout_class=0 gated=0 blocked_active_rollouts=0 assignment_busy=0 observation_gated=0 stable_holds=0 below_threshold=0 deferred=0 no_candidate=0" >/dev/null
...
"${build_dir}/naim-controller" init-db --db "${compute_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${compute_bundle_dir}" --db "${compute_db_path}" --artifacts-root "${compute_artifacts_root}" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${compute_db_path}" --node node-a --runtime-root "${compute_runtime_root}" --state-root "${compute_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${compute_db_path}" --node node-b --runtime-root "${compute_runtime_root}" --state-root "${compute_state_root}" --compose-mode skip >/dev/null
...
"${build_dir}/naim-controller" show-rebalance-plan --db "${compute_db_path}" | grep -F "worker=worker-b placement_mode=movable current=node-b:0 class=gated decision=hold state=gated-target target=node-a:1 action=upgrade-to-exclusive" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${compute_db_path}" | grep -F "gate_reason=compute-pressure" >/dev/null
...
"${build_dir}/naim-controller" show-rebalance-plan --db "${compute_db_path}" | grep -F "gate_reason=observed-insufficient-vram" >/dev/null
"${build_dir}/naim-controller" show-rebalance-plan --db "${compute_db_path}" | grep -F "observation_gated=1" >/dev/null
"${build_dir}/naim-controller" preview-bundle --bundle "${PWD}/config/demo-plane" --node node-a >/dev/null
"${build_dir}/naim-controller" init-db --db "${db_path}" >/dev/null
"${build_dir}/naim-controller" plan-bundle --bundle "${PWD}/config/demo-plane" --db "${db_path}" >/dev/null
"${build_dir}/naim-controller" plan-host-ops --bundle "${PWD}/config/demo-plane" --db "${db_path}" --artifacts-root "${artifacts_root}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${PWD}/config/demo-plane" --db "${db_path}" --artifacts-root "${artifacts_root}" >/dev/null
"${build_dir}/naim-controller" show-node-availability --db "${db_path}" | grep -F "(empty)" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" >/dev/null
"${build_dir}/naim-controller" show-host-health --db "${db_path}" | grep -F "health=unknown" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" >/dev/null
"${build_dir}/naim-controller" serve --db "${db_path}" --listen-host 127.0.0.1 --listen-port "${http_port}" >/tmp/naim-controller-serve.log 2>&1 &
http_server_pid="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${http_port}/api/v1/host-assignments?node=node-a" >/tmp/naim-controller-api.json 2>/dev/null; then
    break
  fi
  sleep 0.1
done
wait_for_http "http://127.0.0.1:${http_port}/api/v1/host-assignments?node=node-a" 100 "/tmp/naim-controller-serve.log"
...
wait "${http_server_pid}" >/dev/null 2>&1 || true
http_server_pid=""
"${build_dir}/naim-controller" render-infer-runtime --db "${db_path}" | grep -F '"gpu_nodes"' >/dev/null
"${build_dir}/naim-controller" render-infer-runtime --db "${db_path}" | grep -F '"serving_workers"' >/dev/null
"${build_dir}/naim-controller" render-compose --db "${db_path}" --node node-a >/dev/null
...
"${script_dir}/naim-devtool.sh" rewrite-infer-runtime-config \
...
"${build_dir}/naim-hostd" show-demo-ops --node node-b >/dev/null
"${build_dir}/naim-hostd" report-observed-state --db "${db_path}" --node node-b --state-root "${state_root}" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-b | grep -F "status=idle" >/dev/null
"${build_dir}/naim-controller" show-host-health --db "${db_path}" --node node-b | grep -F "health=online status=idle" >/dev/null
"${build_dir}/naim-hostd" show-state-ops --db "${db_path}" --node node-a --artifacts-root "${artifacts_root}" --runtime-root "${runtime_root}" --state-root "${state_root}" | grep -F "write-infer-runtime" >/dev/null
"${build_dir}/naim-hostd" apply-state-ops --db "${db_path}" --node node-a --artifacts-root "${artifacts_root}" --runtime-root "${runtime_root}" --state-root "${state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" show-runtime-status --node node-a --state-root "${state_root}" | grep -F "runtime_status: empty" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "disk-runtime-state:" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "disk=plane-alpha-shared node=node-a state=directory-backed" >/dev/null
runtime_shared_root="${runtime_root}/var/lib/naim/disks/planes/alpha/shared"
"${script_dir}/naim-devtool.sh" rewrite-infer-runtime-config \
  --input "${artifacts_root}/alpha/infer-runtime.json" \
...
/mnt/e/dev/Repos/naim-node/runtime/infer/inferctl.sh preload-model --config "${runtime_infer_config}" --alias qwen35 --source-model-id Qwen/Qwen3.5-7B-Instruct --local-model-path "${runtime_root}/var/lib/naim/disks/planes/alpha/shared/models/qwen35" --apply | grep -F "preload-model-plan:" >/dev/null
...
"${build_dir}/naim-hostd" show-runtime-status --node node-a --state-root "${state_root}" | grep -F "launch_ready=no" >/dev/null
"${build_dir}/naim-hostd" report-observed-state --db "${db_path}" --node node-a --state-root "${state_root}" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-a | grep -F "runtime_launch_ready=no runtime_model=Qwen/Qwen3.5-7B-Instruct" >/dev/null
...
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-a | grep -F "cpu_telemetry_source=procfs" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-a | grep -F "net iface=eth0" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-a | grep -F "cpu loadavg=" >/dev/null
...
"${build_dir}/naim-controller" serve --db "${db_path}" --listen-host 127.0.0.1 --listen-port "${http_port}" >/tmp/naim-controller-serve.log 2>&1 &
http_server_pid="$!"
wait_for_http "http://127.0.0.1:${http_port}/api/v1/host-observations?node=node-a" 100 "/tmp/naim-controller-serve.log"
...
"${build_dir}/naim-hostd" show-runtime-status --node node-a --state-root "${state_root}" | grep -F "launch_ready=no" >/dev/null
"${build_dir}/naim-hostd" report-observed-state --db "${db_path}" --node node-a --state-root "${state_root}" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-a | grep -F "runtime_launch_ready=no runtime_model=(empty)" >/dev/null
"${build_dir}/naim-hostd" show-state-ops --db "${db_path}" --node node-b --artifacts-root "${artifacts_root}" --runtime-root "${runtime_root}" --state-root "${state_root}" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=0/3 type=apply-node-state status=pending" >/dev/null
: > "${bad_state_root}"
...
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=1/3 type=apply-node-state status=pending" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-b | grep -F "status=failed" >/dev/null
"${build_dir}/naim-controller" show-events --db "${db_path}" --node node-b | grep -F "category=host-assignment type=failed" >/dev/null
"${build_dir}/naim-controller" plan-bundle --bundle "${PWD}/config/demo-plane" --db "${db_path}" | grep -F "skipped_nodes=node-b(failed)" >/dev/null
if "${build_dir}/naim-hostd" apply-next-assignment --db "${db_path}" --node node-b --runtime-root "${runtime_root}" --state-root "${bad_state_root}" --compose-mode skip >/dev/null 2>&1; then
  echo "check: expected second blocked assignment attempt to fail" >&2
  exit 1
fi
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=2/3 type=apply-node-state status=pending" >/dev/null
if "${build_dir}/naim-hostd" apply-next-assignment --db "${db_path}" --node node-b --runtime-root "${runtime_root}" --state-root "${bad_state_root}" --compose-mode skip >/dev/null 2>&1; then
  echo "check: expected third blocked assignment attempt to fail" >&2
  exit 1
fi
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=3/3 type=apply-node-state status=failed" >/dev/null
"${build_dir}/naim-controller" retry-host-assignment --db "${db_path}" --id 2 >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=3/4 type=apply-node-state status=pending" >/dev/null
...
"${build_dir}/naim-hostd" show-local-state --node node-b --state-root "${state_root}" >/dev/null
"${build_dir}/naim-hostd" show-state-ops --db "${db_path}" --node node-b --artifacts-root "${artifacts_root}" --runtime-root "${runtime_root}" --state-root "${state_root}" >/dev/null
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "disk=worker-b-private node=node-b state=directory-backed" >/dev/null
sqlite3 "${db_path}" \
  "DELETE FROM disk_runtime_state WHERE disk_name='worker-b-private' AND node_name='node-b';"
rm -rf "${runtime_root}/nodes/node-b/var/lib/naim/disks/instances/worker-b/private"
"${build_dir}/naim-hostd" apply-state-ops --db "${db_path}" --node node-b --artifacts-root "${artifacts_root}" --runtime-root "${runtime_root}" --state-root "${state_root}" --compose-mode skip >/dev/null
test -d "${runtime_root}/nodes/node-b/var/lib/naim/disks/instances/worker-b/private"
"${build_dir}/naim-controller" show-state --db "${db_path}" | grep -F "disk=worker-b-private node=node-b state=directory-backed-fallback" >/dev/null
"${build_dir}/naim-controller" show-disk-state --db "${db_path}" --node node-b | grep -F "disk=worker-b-private kind=worker-private node=node-b" >/dev/null
"${build_dir}/naim-controller" show-disk-state --db "${db_path}" --node node-b | grep -F "realized_state=directory-backed-fallback" >/dev/null
"${build_dir}/naim-controller" show-disk-state --db "${db_path}" --node node-b | grep -F "worker-b-private" | grep -F "usage_bytes" >/dev/null
"${build_dir}/naim-controller" show-disk-state --db "${db_path}" --node node-b | grep -F "worker-b-private" | grep -F "read_bytes=" >/dev/null
"${build_dir}/naim-controller" show-disk-state --db "${db_path}" --node node-b | grep -F "worker-b-private" | grep -F "perf_counters=no" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "attempts=4/4 type=apply-node-state status=applied" >/dev/null
"${build_dir}/naim-controller" show-host-observations --db "${db_path}" --node node-b | grep -F "status=applied applied_generation=1" >/dev/null
"${build_dir}/naim-controller" show-host-health --db "${db_path}" --node node-b | grep -F "health=online status=applied applied_generation=1" >/dev/null
"${build_dir}/naim-controller" set-node-availability --db "${db_path}" --node node-b --availability unavailable --message "check maintenance" >/dev/null
"${build_dir}/naim-controller" show-node-availability --db "${db_path}" --node node-b | grep -F "availability=unavailable" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "type=drain-node-state status=pending" >/dev/null
"${build_dir}/naim-controller" plan-bundle --bundle "${PWD}/config/demo-plane" --db "${db_path}" | grep -F "skipped_nodes=node-b(unavailable)" >/dev/null
"${build_dir}/naim-controller" set-node-availability --db "${db_path}" --node node-b --availability active --message "back online" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${db_path}" --node node-b | grep -F "generation=1 attempts=0/3 type=apply-node-state status=pending" >/dev/null

"${build_dir}/naim-controller" init-db --db "${parallel_db_path}" >/dev/null
"${build_dir}/naim-controller" apply-bundle --bundle "${PWD}/config/demo-plane" --db "${parallel_db_path}" --artifacts-root "${parallel_artifacts_root}" >/dev/null
(
  "${build_dir}/naim-hostd" apply-next-assignment --db "${parallel_db_path}" --node node-a --runtime-root "${parallel_runtime_root}" --state-root "${parallel_state_root}" --compose-mode skip >/dev/null
) &
parallel_pid_a=$!
(
  "${build_dir}/naim-hostd" apply-next-assignment --db "${parallel_db_path}" --node node-b --runtime-root "${parallel_runtime_root}" --state-root "${parallel_state_root}" --compose-mode skip >/dev/null
) &
parallel_pid_b=$!
wait "${parallel_pid_a}"
wait "${parallel_pid_b}"
"${build_dir}/naim-controller" show-host-assignments --db "${parallel_db_path}" --node node-a | grep -F "status=applied" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${parallel_db_path}" --node node-b | grep -F "status=applied" >/dev/null

"${build_dir}/naim-controller" init-db --db "${multiplane_db_path}" >/dev/null
mkdir -p "${multiplane_beta_bundle}/workers"
cat > "${multiplane_beta_bundle}/plane.json" <<'JSON'
{
  "name": "beta",
  "control_root": "/naim/shared/control/beta",
  "runtime": {
    "primary_infer_node": "node-a",
    "net_if": "eth0",
    "models_root": "/naim/shared/models",
...
JSON
"${build_dir}/naim-controller" import-bundle --db "${multiplane_db_path}" --bundle "${PWD}/config/demo-plane" >/dev/null
"${build_dir}/naim-controller" import-bundle --db "${multiplane_db_path}" --bundle "${multiplane_beta_bundle}" >/dev/null
"${build_dir}/naim-controller" list-planes --db "${multiplane_db_path}" | grep -F "name=alpha state=ready generation=1" >/dev/null
"${build_dir}/naim-controller" list-planes --db "${multiplane_db_path}" | grep -F "name=beta state=ready generation=1" >/dev/null
"${build_dir}/naim-controller" start-plane --db "${multiplane_db_path}" --plane alpha >/dev/null
"${build_dir}/naim-controller" start-plane --db "${multiplane_db_path}" --plane beta >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${multiplane_db_path}" --node node-a | grep -F "plane=alpha" >/dev/null
"${build_dir}/naim-controller" show-host-assignments --db "${multiplane_db_path}" --node node-a | grep -F "plane=beta" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-a --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-a --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-b --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-b --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
...
"${build_dir}/naim-hostd" show-local-state --node node-a --state-root "${multiplane_state_root}" | grep -F "instances=3" >/dev/null
"${build_dir}/naim-hostd" show-local-state --node node-b --state-root "${multiplane_state_root}" | grep -F "instances=2" >/dev/null
"${build_dir}/naim-controller" stop-plane --db "${multiplane_db_path}" --plane alpha >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-a --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip | grep -F "assignment_type=stop-plane-state" >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-b --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip | grep -F "assignment_type=stop-plane-state" >/dev/null
"${build_dir}/naim-controller" show-plane --db "${multiplane_db_path}" --plane alpha | grep -F "state=stopped" >/dev/null
"${build_dir}/naim-controller" show-plane --db "${multiplane_db_path}" --plane beta | grep -F "state=running" >/dev/null
"${build_dir}/naim-hostd" show-local-state --node node-a --state-root "${multiplane_state_root}" | grep -F "instances=1" >/dev/null
"${build_dir}/naim-hostd" show-local-state --node node-b --state-root "${multiplane_state_root}" | grep -F "instances=1" >/dev/null
"${build_dir}/naim-controller" start-plane --db "${multiplane_db_path}" --plane alpha >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-a --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-hostd" apply-next-assignment --db "${multiplane_db_path}" --node node-b --runtime-root "${multiplane_runtime_root}" --state-root "${multiplane_state_root}" --compose-mode skip >/dev/null
"${build_dir}/naim-controller" show-plane --db "${multiplane_db_path}" --plane alpha | grep -F "state=running" >/dev/null
"${build_dir}/naim-hostd" show-local-state --node node-a --state-root "${multiplane_state_root}" | grep -F "instances=3" >/dev/null
"${build_dir}/naim-hostd" show-local-state --node node-b --state-root "${multiplane_state_root}" | grep -F "instances=2" >/dev/null

test -d "${runtime_root}/var/lib/naim/disks/planes/alpha/shared"
test -f "${runtime_root}/var/lib/naim/disks/planes/alpha/shared/control/alpha/infer-runtime.json"
...
test -f "${runtime_root}/var/lib/naim/disks/planes/alpha/shared/control/alpha/runtime-status.json"
test -d "${runtime_root}/nodes/node-b/var/lib/naim/disks/instances/worker-b/private"
